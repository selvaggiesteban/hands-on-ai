...
| :--- | :--- | :--- | :--- |
| **`agents/`** | **Intelligence Layer.** Defines the persona, capabilities, and strict mandates for AI agents. | 17 Areas | Markdown (`agent.md`) |
| **`checklist/`** | **Quality Assurance Layer.** Passive validation gates that MUST be passed before any deliverable is approved. | 17 Areas | Markdown (`checklist.md`) |
| **`skills/`** | **Capability Layer.** Detailed technical guides and reference materials required for execution. | 17 Areas | Markdown (`skill.md`) |
| **`scripts/`** | **Automation Layer.** Code and documentation for executing tasks within each domain. | 17 Areas | Mixed (`.py`, `.md`) |
| **`plan/`** | **Strategy Layer.** 17 Strategic Operational Plans connecting the resources above into executable roadmaps. | 17 Docs | Markdown (`.md`) |
...
2.  **`automation`**: Internal system automation and tooling.
3.  **`automation_tooling`**: specialized infrastructure for agent orchestration.
4.  **`core`**: Foundational agency logic and optimization.
...
If the "Absolute Truths 2026" (Global Standards) change, run this script to propagate updates to all Agents and Skills documents:
```bash
...
**Maintained by:** Hands-on AI Automated Systems
