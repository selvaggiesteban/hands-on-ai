...
if __name__ == "__main__":
    ...
    try:
        ...
        if os.path.exists(project_meta_path):
            print("??  Advertencia: Ya existe una configuraci�n 'project_meta' en este directorio.")
            action = input("�Qu� deseas hacer? ([S]obrescribir, [F]usionar/Actualizar, [C]ancelar): ").lower()
            if action == 's':
                ...
                print("Fusionando/Actualizando (funcionalidad pendiente). Por ahora, se cancelar�.")
                return
            else:
        ...
        if not os.path.exists(template_path):
             print(f"? Error: No se encontr� la plantilla de 'project_meta' en {template_path}")
             return
        ...
        # 3. Escaneo Aut�nomo
        print("\n?? Escaneando archivos del proyecto para an�lisis...")
        project_files = []
        ...
        if confirm != 's':
            print("Operaci�n cancelada. 'project_meta' ha sido creado pero est� vac�o.")
            return

        # 4. Invocaci�n del Equipo de Agentes
        ...
        task_description = f"""
        **Tarea Cr�tica: Inicializaci�n de Proyecto**
        ...
        **Contexto del Proyecto (Resumen de Archivos):**
        {files_context}
        **Tu Misi�n:**
        Como 'orchestrator_agent', coordina a tu equipo de 11 especialistas (Product Manager, Business Analyst, Cloud Architect, etc.) para que cada uno, seg�n su rol, analice el contexto proporcionado y rellene los documentos correspondientes dentro de la nueva carpeta '{project_meta_path}'.
        **Instrucciones Finales:**
        - Utiliza las herramientas write_file para actualizar cada documento de project_meta.
        - Si la informaci�n no es suficiente, rellena los campos con placeholders como [TODO: Definir por el usuario].
        - Al finalizar, responde �nicamente con "Proceso de inicializaci�n de project_meta completado."
        """
